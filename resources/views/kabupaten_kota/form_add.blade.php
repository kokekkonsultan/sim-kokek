...
<form action='{{ url('kabupaten-kota') }}' method='post'>
@csrf

    <div class="mb-3 row">
        <label for="nama_kota_kab_indonesia" class="col-sm-2 col-form-label">Nama Kabupaten/ Kota <span class="text-danger">*</span></label>
        <div class="col-sm-10">
            <input type="text" class="form-control" name='nama_kota_kab_indonesia' value="{{ Session::get('nama_kota_kab_indonesia') }}" id="nama_kota_kab_indonesia">
        </div>
    </div>
        ...
        <div class="col-sm-10">
            <select name="provinsi" id="provinsi" class="form-control" required>
            	...
            	@foreach ($provinsi as $val)
            		<option value="{{ $val->id_provinsi_indonesia }}">{{ $val->nama_provinsi_indonesia }}</option>
            	@endforeach
            </select>
        </div>
    </div>
