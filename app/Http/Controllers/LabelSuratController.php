...
class LabelSuratController extends Controller
{
    /**
    ...
    {
        $this->data = [];
        $this->data['title'] = "Cetak Label Surat";
        ...
        if ($request->ajax()) {
            return Datatables::of($organisasi)
                ...
                ->addColumn('select_organisasi', function ($row) {
                    ...
                    $select_organisasi = '<div class="checkbox-list"><label class="checkbox"><input type="checkbox" name="id_organisasi_surat_ditujukan[]" value="' . $row->id_organisasi_surat_ditujukan . '" class="child"><span></span>' . trim($ins) . '</label></div><span class="form-text text-muted">' . $row->email . '</span>';
        ...
        return view('label_surat.index', $this->data);
    }
    ...
    public function buat_label(Request $request)
    {
        ...
        $data_organisasi = collect(DB::select("SELECT *,
        ...
        WHERE id IN ($id_organisasi_surat_ditujukan)"));



        //Cek apakah submit dengan value get email atau tidak
        if ($request['is_submit'] == 'get_email') {
            $this->data = [];
            $this->data['title'] = 'Email Organisasi';
            ...
            foreach ($data_organisasi as $row) {
                ...
                $arr_table[] = '<tr>
                                    <td>' . $no++ . '</td>
                                    <td>' . $row->nama_organisasi_utama . '</td>
                                    <td>' . $row->email . '</td>
                                </tr>';
            }
            $this->data['email'] = implode("", $arr_email);
            $this->data['table'] = implode("", $arr_table);

            return view('label_surat.form_email', $this->data);
            ...
            $pdf->SetAuthor('Hanif');
            $pdf->SetTitle('Label Surat');
            $pdf->SetSubject('LABEL SURAT');
            $pdf->SetKeywords('LABEL SURAT, KOP SURAT');


            if (in_array($request['is_submit'], [1, 2])) {
                $page_format = ['MediaBox' => array('llx' => 0, 'lly' => 0, 'urx' => 60, 'ury' => 40)];
            } elseif (in_array($request['is_submit'], [3, 4])) {
                $page_format = ['MediaBox' => array('llx' => 0, 'lly' => 0, 'urx' => 70, 'ury' => 40)];
            } elseif (in_array($request['is_submit'], [5, 6])) {
                $page_format = ['MediaBox' => array('llx' => 0, 'lly' => 0, 'urx' => 70, 'ury' => 50)];
            } else {
                $page_format = ['MediaBox' => array('llx' => 0, 'lly' => 0, 'urx' => 100, 'ury' => 50)];
            }
            ...
            foreach ($data_organisasi as $row) {
                ...
                //NO TELEPON
                if (in_array($request['is_submit'], [2, 4, 6, 8])) {
                    if ($row->telepon != '') {
